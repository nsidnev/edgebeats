defmodule LiveBeatsWeb.PlayerLive do
  ...
  def mount(_params, _session, socket) do
    ...
    profile_id =
      case current_user.active_profile_user do
        %Accounts.User{id: id} ->
          id
  ...
  defp assign_profile(socket, profile)
    ...
    assign(socket,
      profile: profile,
      own_profile?: not is_nil(profile) && MediaLibrary.owns_profile?(current_user, profile)
    )
  end
  ...
  def handle_event("next_song", _event_data, socket) do
  ...
  def handle_event("prev_song", _event_data, socket) do
    %{song: song, current_user: current_user} = socket.assigns
